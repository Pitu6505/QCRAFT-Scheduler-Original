...
class executeCircuitIBM:
    ...
    def _load_service(self) -> QiskitRuntimeService:
        """
        Load IBM Quantum credentials from environment variables when available.

        Supported variables:
            IBM_TOKEN, IBM_CRN, IBM_INSTANCE

        If they are not set, the method falls back to the default Qiskit saved account.
        """
        token = os.getenv('IBM_TOKEN')
        crn = os.getenv('IBM_CRN') or os.getenv('IBM_INSTANCE')

        try:
            if token and crn:
                return QiskitRuntimeService(channel='ibm_cloud', token=token, instance=crn)
            return QiskitRuntimeService(channel='ibm_cloud')
        except Exception as exc:
            raise RuntimeError(
                'No se pudo cargar la cuenta de IBM Cloud. '
                'Define IBM_TOKEN e IBM_CRN en QCRAFT-Scheduler/db/.env '
                'o guarda la cuenta con QiskitRuntimeService.save_account(..., channel="ibm_cloud").'
            ) from exc
